...
export default function Track(
  ...
  function setRange() {
    const rangeOption = carousel.options.range;
    const loop = carousel.options.loop;
    ...
    min =
      minIdx === -infinity
        ? minIdx
        : carousel.track.idxToDist(minIdx || 0, true, 0);
    max = maxIdx === infinity ? maxIdx : idxToDist(maxIdx, true, 0);
    if (dragMax === null) loopMax = maxIdx;
    if (
      getProp(rangeOption, 'align', false) &&
      maxIdx !== infinity &&
      carouselData[absToRel(maxIdx)][2] === 0
    ) {
      max = max - (1 - carouselData[absToRel(maxIdx)][0]);
  ...
  function details(): TrackDetails {
    ...
    const viewportPosition = positionMod - carouselData[0][2];
    const carouselDataStart =
      0 -
    ...
    const activeOrigin = carouselData[rel][2];
    const slideDetails = carouselData.map((slide, idx) => {
      let distanceViewport = carouselDataStart + sumLength;
      ...
      if (loop) {
        if (signIdxDistance === -1 && distanceViewport > activeOrigin)
          absoluteIndex += carouselDataCount;
        if (signIdxDistance === 1 && distanceViewport < activeOrigin)
          absoluteIndex -= carouselDataCount;
    ...
    return {
      ...
      rel,
      carouselData: slideDetails,
      carouselDataLength: length,
    };
  }
  ...
  function getIndexes(pos) {
    ...
    if (origin === carouselDataCount) {
      idx = 0;
      factor += positionSign > 0 ? 1 : -1;
    }
    const abs = idx + factor * carouselDataCount * positionSign;
    return {
      abs,
      origin,
      rel: idx,
  ...
  function idxToDist(idx, absolute, fromPosition) {
    ...
    const otherDistance = absoluteIdxToDist(
      nextIdx - carouselDataCount * sign(idxDistance)
    );
    if (
      (otherDistance !== null &&
  ...
  function absoluteIdxToDist(idx, fromPosition?) {
    ...
    const distance =
      Math.floor((idx - (abs - rel)) / carouselDataCount) * length;
    return round(
      distanceToStart -
        positionRelative -
        distanceToStart +
        ...
        distance +
        (origin === carouselDataCount ? length : 0)
    );
  }
  ...
  function initcarouselData() {
    ...
    carouselData = (opts.trackConfig || []).map((entry) => [
    ...
    const lastIdx = carouselDataCount - 1;
    trackLength = round(
      length +
        carouselData[0][2] -
        carouselData[lastIdx][0] -
        carouselData[lastIdx][2] -
        carouselData[lastIdx][1]
    );
    let lastDistance;
  ...
  function absToRel(idx) {
    return ((idx % carouselDataCount) + carouselDataCount) % carouselDataCount;
  }
  ...
  function to(value) {
    ...
    if (idx !== currentIdx) {
      ...
      currentIdx = idx;
      if (emitEvent) carousel.emit('slideChanged');
    }
  }
  ...
  function trackUpdate(unset?: boolean) {
    ...
    carousel.emit('detailsChanged');
    return newDetails;
  }

  function init(index) {
    initcarouselData();
    if (!carouselDataCount) return trackUpdate(true);
    setRange();
    if (isNumber(index)) {
